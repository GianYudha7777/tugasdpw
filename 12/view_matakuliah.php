...
<html lang="id">
<head>
    <meta charset="UTF-8"><title>Data Mata Kuliah - OOP</title>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-slate-600">
                    ...
                    if (isset($_GET['keyword']) && $_GET['keyword'] != "") {
                        ...
                        $result = $con->query("SELECT * FROM t_matakuliah ORDER BY kodeMK ASC");
                    }
                    
                    while ($data = $result->fetch_assoc()) { ?>
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="py-4 px-6 font-semibold text-slate-900"><?= $data['kodeMK']; ?></td>
                            <td class="py-4 px-6 font-medium"><?= htmlspecialchars($data['namaMK']); ?></td>
                            <td class="py-4 px-6"><span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700"><?= htmlspecialchars($data['sks']); ?> SKS</span></td>
                            <td class="py-4 px-6 text-slate-500"><?= htmlspecialchars($data['jam']); ?> Jam</td>
